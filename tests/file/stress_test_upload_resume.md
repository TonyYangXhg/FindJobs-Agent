...
- [x] **[必选] T1** 搭环境：安装 Locust、准备一份固定小体积测试 PDF、确认 `python api_server.py` 可启动、建立 `tests/results/`；记录「优化前」为同步阻塞上传（请求线程内跑完 `parse_resume`）
- [x] **[必选] T2** 写并启动 Mock LLM 上游（固定 ~800ms 延迟 + 预设可被解析器接受的 JSON），用环境变量把 `LLM_API_URL` 指过去（**禁止**用真实 DeepSeek/OpenAI 做本方案压测）
- [x] **[必选] T3** 建立 Baseline：用 Locust 压测 **重点 A：`POST /api/resume/upload`**；记录「HTTP 返回耗时」「解析完成总耗时」（优化前二者相同）、错误率
- [x] **[必选] T4** 做最小优化（详见 `优化计划_v1_upload_resume.md`）：**必选落地**上传异步化（立即返回 task_id）+ 任务状态查询、`resumes_store` 锁 + 原子落盘
- [x] **[必选] T5** 同场景复测：相同 Locust 参数再跑一遍，填「优化前 → 优化后」对比表（**必须分开填「受理 P95」与「端到端完成 P95」**）——1u/5u 已填
- [x] **[必选] T7** 整理简历素材：1 张对比表 + 2～3 句「约束 → 方案 → 数字」结论；把 Locust 命令、参数、CSV/截图放进 `tests/results/`
...
![](./../pic/upload_resume/stress_1_优化前.png)

Figure 1.2 优化前 user=1

![](./../pic/upload_resume/stress_2_优化前.png)

Figure 1.3 优化前 user=5

![](./../pic/upload_resume/stress_1_优化前_3m.png)

Figure 1.4 优化前 user=5

![](./../pic/upload_resume/stress_2_优化前_3m.png)
...
| 指标 | 优化前（实测） | 优化后（Locust 2026-09-06） | 提升 |
|------|----------------|-----------------------------|------|
| 上传受理 P95（1u） | **17000 ms** | **9 ms** | 约 **1800×** |
| 上传受理 RPS（1u） | **≈0.04** | **≈0.29** | 约 **7×** |
| 端到端轮询单次 P95（1u） | （与受理捆绑） | **4 ms** | 受理与完成已分离 |
| 旁路 jobs P95（1u） | **13000 ms**（Median 4） | **13000 ms**（Median **4**） | 冷启动尖刺；upload 已不堵 |
| 上传受理 P95（5u） | **18000 ms** | **30 ms** | 约 **600×** |
| 上传受理 RPS（5u） | **≈0.1** | **≈1.1** | 约 **11×** |
| 端到端轮询单次 P95（5u） | — | **45 ms** | 毫秒级轮询 |
| 旁路 jobs P95（5u） | **4 ms** | **45 ms** | 仍亚百毫秒；加压下仍可用 |
| 上传错误率（1u / 5u） | **0% / 0%** | **0% / 0%**（0/35、0/204） | 持平 |
