...
| 场景 | Users | 指标类型 | 接口 | RPS | P95 (ms) | 错误率 |
|------|-------|----------|------|-----|----------|--------|
| 优化前 | 1 | 受理(=端到端) | POST /api/resume/upload |  |  |  |
| 优化前 | 1 | 旁路短接口 | GET /api/jobs?page=1&page_size=50 |  |  |  |
| 优化前 | 5 | 受理(=端到端) | POST /api/resume/upload |  |  |  |
| 优化前 | 5 | 旁路短接口 | GET /api/jobs?page=1&page_size=50 |  |  |  |
...
| 指标 | 优化前（典型） | 优化后期望 |
|------|----------------|------------|
| 上传受理 P95 | 约 2×Mock延迟～数十秒（串行 2 次 LLM） | **&lt;200ms**（立即 202） |
| 端到端完成 P95（Mock 800ms×2） | 与受理相同 | 并行后约 **~0.8s+业务**；串行约 **~1.6s+业务** |
| 旁路 jobs P95（5 并发上传时） | 明显抬升甚至失败 | 接近无上传压力时的水平 |
